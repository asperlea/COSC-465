...
class Router(object):
	...
	def examineStalled(self, arp_ip):
		...
		for dst in keys:
			...
			if dst in self.ip_to_ether: #We've since figured this one out
				...
				for ether_pkt in stalled.getList():
				...
				if difference/stalled.tries > 1:
					if stalled.tries >=5: #Timeout, call it a day on this packet
						continue
					else:
	...
	def router_main(self):
		...
		while True:
			try:
				self.examineStalled(arp_ip) #deal with stalled waiting on ARPs
				...
				if pkt.type == pkt.ARP_TYPE: #Is an ARP
					...
					if dst_ip in self.ip_to_ether: #We have the mapping dst_ip->eth
						ether.dst = self.ip_to_ether[dst_ip]
						self.net.send_packet(name, ether) #Send packet on its way
					else:					
						if dst_ip in arp_ip: #Already waiting on ARP for this
							arp_ip[dst_ip].addPacket(ether)
						else: #New IP to ARP at
							request	 = self.makeRequest(dst_ip, src_ip, src_eth) #create ARP req
							waiter = arpWaiter(name, request, ether)
							arp_ip[dst_ip] = waiter
